...
inside a createos-sandbox sandbox, expose it via HTTP ingress, and verify the Control UI and
...
1. Creates a `s-1vcpu-2gb` createos-sandbox sandbox with `ingress_enabled: true`.
...
5. Starts the OpenClaw gateway in foreground mode via `nohup setsid` (no systemd in
   createos-sandbox; `;` before `nohup` prevents `runCommand` from holding the stdout pipe).
...
| Primitive                   | SDK call                                              |
| --------------------------- | ----------------------------------------------------- |
| Sandbox create with ingress | `box.createSandbox({ ingress_enabled: true })`         |
