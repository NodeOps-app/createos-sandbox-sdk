...
3. Daemonises the Pushgateway on `0.0.0.0:9091` using `nohup setsid` (no
   systemd in devbox) — binding `0.0.0.0` is required for createos-sandbox ingress
   forwarding.
...
6. Scrapes the public `/metrics` ingress URL until the metric appears,
   confirming the full push→scrape round-trip over the createos-sandbox ingress layer.
...
| primitive                      | SDK call                                      |
| ------------------------------ | --------------------------------------------- |
| Boot with HTTP ingress         | `box.createSandbox({ ingress_enabled: true })` |
