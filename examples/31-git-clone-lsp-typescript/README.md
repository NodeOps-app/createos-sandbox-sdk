...
2. Installs `git` and clones `microsoft/TypeScript-Node-Starter` at `--depth=1`.
...
5. Runs the driver inside the sandbox; it opens an LSP session over stdio:
   ...
   - Requests `textDocument/definition` at the first identifier on line 0.
   - Requests `textDocument/completion` at a mid-file position.
