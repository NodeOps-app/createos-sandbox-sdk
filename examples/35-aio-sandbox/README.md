...
Combines every core createos-sandbox SDK primitive — sandbox creation, file upload,
...
```sh
cp .env.example .env
# fill in CREATEOS_SANDBOX_BASE_URL, CREATEOS_SANDBOX_API_KEY, ANTHROPIC_BASE_URL, ANTHROPIC_AUTH_TOKEN, ANTHROPIC_MODEL
source .env
bun index.ts
```
...
1. Prompts an LLM (via `@anthropic-ai/sdk`) to generate a self-contained HTML
   page that reports running inside a createos-sandbox sandbox.
2. Creates a createos-sandbox sandbox (`s-2vcpu-2gb`, `devbox:1`) with `ingress_enabled:
...
5. Daemonises `python3 -m http.server 8000 --directory /srv --bind 0.0.0.0`
   via `nohup setsid` (no systemd in createos-sandbox VMs; `;` before `nohup` prevents the
...
| Primitive                            | SDK call                                      |
| ------------------------------------ | --------------------------------------------- |
| Sandbox creation with public ingress | `box.createSandbox({ ingress_enabled: true })` |
