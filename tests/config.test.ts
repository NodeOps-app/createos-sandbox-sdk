...
describe("resolveConfig — sources and precedence", () => {
  ...
  test("falls back to CREATEOS_SANDBOX_API_KEY / CREATEOS_SANDBOX_BASE_URL when options are absent", () => {
    process.env.CREATEOS_SANDBOX_API_KEY = "sk_env";
    process.env.CREATEOS_SANDBOX_BASE_URL = "https://env.test";
...
describe("resolveConfig — validation", () => {
  ...
  test("rejects an unparseable base URL", () => {
    expect(() => resolveConfig({ baseUrl: "not-a-url", fetch: fetchStub })).toThrow(
      CreateosSandboxError,
    );
  });
