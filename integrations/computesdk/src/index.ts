 ...
 * Thin adapter over the official `createos-sandbox-sdk` npm package — the same
 * SDK used across the CreateOS tooling.
 *
 * Design notes:
 *  - `TSandbox` is the native `createos-sandbox-sdk` `Sandbox` handle (not a wrapper),
...
import { sleep } from "createos-sandbox-sdk";
...
export const createosSandbox = defineProvider<Sandbox, CreateosConfig>({
  ...
  methods: {
    sandbox: {
      ...
      // Escape hatch: native createos-sandbox-sdk handle (pause/resume/fork/disks/...).
...
export type { Sandbox as CreateosSandbox } from "createos-sandbox-sdk";
