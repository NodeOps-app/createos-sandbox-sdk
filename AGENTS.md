...
**You are in `fc-sdk` — the public TypeScript SDK (+ `examples/`).** The `fc`
control plane upstream defines the wire contract this mirrors in `types.ts`;
keep `examples/` in sync (see Conventions) and check CLI / docs / plugin parity
for any new public method. This repo is one of five in the product mesh.
